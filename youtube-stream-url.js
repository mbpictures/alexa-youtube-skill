...
const getInfo = async ({url}) => {
    ...
    let ytApi = 'http://www.youtube.com/get_video_info?video_id=' + video_id;

    let response = await axios.get(ytApi).catch((err) => ({data: err}));

    if (!response.data/* || response.data.indexOf('errorcode') > -1*/){
		console.log(response.data.indexOf('errorcode'));
		return false;
	} 
